...
dSOC_models.columns = model_labels['cmip5'] + model_labels['cmip6']
# dSOC_models.to_pickle('dSOC_models')

fig,ax = plt.subplots(figsize=(20,8))
dSOC_models.boxplot(fontsize=20)
plt.ylabel('$\Delta$SOC (Pg)',fontsize=25)
plt.xlabel('RF-Models',fontsize=25)
plt.xticks(rotation='vertical',fontsize=18)
[ax.get_xticklabels()[i].set_color("blue") for i in np.arange(0,12)]
[ax.get_xticklabels()[i].set_color("r") for i in np.arange(12,24)]    
...
fig,ax = plt.subplots(figsize=(20,8))
dSOC_input.boxplot(fontsize=20)
plt.ylabel('$\Delta$SOC (Pg)',fontsize=25)
plt.xlabel('RF-Inputs',fontsize=25)
plt.xticks(rotation='vertical',fontsize=18)
[ax.get_xticklabels()[i].set_color("blue") for i in np.arange(0,12)]
[ax.get_xticklabels()[i].set_color("r") for i in np.arange(12,24)]
fig.savefig('./figures/fig_uncertainty_among_inputs_delta_soc.png',bbox_inches='tight',dpi=300)
...
ax1.hist(dSOC_models.values.flatten(),bins=10, density =True, alpha=0.25,label = 'RF-$\Delta$SOC')
xmin, xmax = plt.xlim()
...
axins.pie(sizes, explode=explode, labels=labels, autopct='%1.1f%%',textprops={'fontsize': 16},
...
data = dSOC_models.values.copy()
np.savetxt("dSOC_models.csv",data, delimiter=",")
...
data = dSOC_models.values[0:12,0:12]
# data = data_.copy()
# data = data_[8:16,8:16]
n=12 #n=16
...
# fig1, ax1 = plt.subplots()
axins = inset_axes(ax2, width=2.5, height=2.5,loc='upper left',bbox_to_anchor=(0.08,-0.1,2,1.075), bbox_transform=ax1.transAxes)
axins.pie(sizes, explode=explode, labels=labels, autopct='%1.1f%%',textprops={'fontsize': 14},
        shadow=False, startangle=90)

data = dSOC_models.values[12:24,12:24]
# data = data_.copy()
# data = data_[8:16,8:16]
n=12 #n=16
...
# fig1, ax1 = plt.subplots()
axins = inset_axes(ax2, width=2.5, height=2.5,loc='upper right',bbox_to_anchor=(0.08,-0.1,2,1.075), bbox_transform=ax1.transAxes)
axins.pie(sizes, explode=explode, labels=labels, autopct='%1.1f%%',textprops={'fontsize': 14},
        shadow=False, startangle=90)

""" end """

fig.savefig('./figures/FIG_histogram_cmip56_delta_soc.png',bbox_inches='tight',dpi=300)

#%%

fig,ax = plt.subplots(figsize=(10,8))
# 
_, bins, _ = ax.hist(data.flatten(), 20, density =1, alpha=0.5)
mu, sigma = scipy.stats.norm.fit(data.flatten())
best_fit_line = scipy.stats.norm.pdf(bins, mu, sigma)
ax.plot(bins, best_fit_line,'blue',label='$\Delta$SOC (ML)')
y = [0.01]*16
ax.scatter(data_000,y,c='black',s=50,label = '$\Delta$SOC (Obs-constrained)')
box = ax.boxplot(data_000,vert=False,positions=[0.01],widths=0.0015,
                 whiskerprops=dict(linestyle='--'),boxprops=dict(linestyle='--'),showfliers=False)
...
y = [0.012]*16
data_original = [data[i,i] for i in np.arange(16)]
ax.scatter(data_original,y,c='red',s=50,label = '$\Delta$SOC (CMIP5&6)')
box = ax.boxplot(data_original,vert=False,positions=[0.012],widths=0.0015,
                 whiskerprops=dict(linestyle='--'),boxprops=dict(linestyle='--'),showfliers=False)
...
ax.set_ylim([0,0.014])
ax.set_yticks(np.arange(0,0.016,0.002))
ax.set_xticks(np.arange(-200,350,100))
ax.set_xticklabels(np.arange(-200,350,100),fontsize=18)
ax.set_yticklabels(np.arange(0,0.016,0.002),fontsize=18)
ax.set_ylabel('Probablity',fontsize=18)
ax.set_xlabel('$\Delta$SOC (Pg)',fontsize=18)

...
# models = ['bcc']
# models = ['bcc','bcc','can_esm','can_esm','cesm','cesm','ipsl','ipsl',
#                     'miroc','miroc','mpi','mpi','ncc','ncc','mohc_ukesm','mohc_ukesm']
models = ['bcc','can_esm','cesm','ipsl','miroc','mpi','ncc','mohc_ukesm']
for model in models:
    ...
    for cmip in cmips:
        ...
        for i in np.arange(len(lats)):
            rows = np.argmin((lats[i] - lats_lc)**2)
            for j in np.arange(len(lons)):
                cols = np.argmin((lons[j]- lons_lc)**2)        
                ...
                if cSoil[i,j]>0: 
                    ...
                    model_lc[i,j] = unique[np.argmax(counts)]                       
        # ============================================================================================================  
        ...
        for i in np.arange(2,10):
            ...
            df_fi_biome[lc_types[i-2]] = feature_importances_mean
            f_imp = pd.DataFrame({'features':feature_names,'scores':feature_importances_mean})
            f_imp = f_imp.sort_values(by='scores',ascending=False)
        ...
        fig = df_fi_biome.set_index('features').T.plot(
            kind='bar', stacked=True,legend=False,grid=False,ax=axes1[m//4,m%4])
        axes1[m//4,m%4].set_title(model_id[m],fontsize=12)
        axes1[m//4,m%4].set(xticklabels=[])
        if m//4>=3:
            axes1[m//4,m%4].set_xticklabels(lc_types,rotation = 45, ha="right")
        
        if m%4==0:
            axes1[m//4,m%4].set_ylabel('Relative importance',fontsize=12)
        m+=1
...
fig1.legend(labels, ncol=9, 
            bbox_to_anchor=(0.075, 0.925, 0.75, .10), loc='lower left',mode="expand") #
# 0.1, 0.9, 0.75, .10
...
fig1.savefig('./figures/20221216_fig_relative_importance_biomes_CMIP5&6.png',bbox_inches='tight',dpi=300)  
